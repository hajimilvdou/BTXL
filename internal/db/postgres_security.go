...
// ============================================================
// PostgresStore — SecurityStore 接口实现
...
func (s *PostgresStore) CreateIPRule(ctx context.Context, rule *IPRule) error {
	const query = `
...
// 按创建时间降序排列（最新规则优先）
func (s *PostgresStore) ListIPRules(ctx context.Context) ([]*IPRule, error) {
	const query = `
...
// 删除不存在的记录时静默成功（幂等操作）
func (s *PostgresStore) DeleteIPRule(ctx context.Context, id int64) error {
	const query = `DELETE FROM ip_rules WHERE id = $1`
...
func (s *PostgresStore) CreateRiskMark(ctx context.Context, mark *UserRiskMark) error {
	const query = `
...
func (s *PostgresStore) GetActiveRiskMarks(ctx context.Context, userID int64) ([]*UserRiskMark, error) {
	const query = `
...
// 支持按 user_id 和活跃状态筛选
func (s *PostgresStore) ListRiskMarks(ctx context.Context, opts ListRiskMarksOpts) ([]*UserRiskMark, int64, error) {
	var conditions []string
	var args []any
	paramN := 1

	if opts.UserID != nil {
		conditions = append(conditions, fmt.Sprintf("user_id = $%d", paramN))
	...
	dataQuery := fmt.Sprintf(
		`SELECT id, user_id, mark_type, reason, marked_at, expires_at, auto_applied
		 FROM risk_marks %s ORDER BY marked_at DESC LIMIT $%d OFFSET $%d`,
		where, paramN, paramN+1,
	)
...
func (s *PostgresStore) ExpireRiskMarks(ctx context.Context) (int64, error) {
	const query = `DELETE FROM risk_marks WHERE expires_at <= NOW()`
...
func (s *PostgresStore) RecordAnomalyEvent(ctx context.Context, event *AnomalyEvent) error {
	const query = `
...
// 支持按 user_id 和时间范围筛选
func (s *PostgresStore) ListAnomalyEvents(ctx context.Context, opts ListAnomalyEventsOpts) ([]*AnomalyEvent, int64, error) {
	var conditions []string
	var args []any
	paramN := 1
	...
	if opts.After != nil {
		conditions = append(conditions, fmt.Sprintf("created_at > $%d", paramN))
...
func (s *PostgresStore) ListAuditLogs(ctx context.Context, opts ListAuditLogsOpts) ([]*AuditLog, int64, error) {
	var conditions []string
	var args []any
	paramN := 1
	...
	if opts.Action != nil {
		conditions = append(conditions, fmt.Sprintf("action = $%d", paramN))
